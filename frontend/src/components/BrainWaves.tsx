...
export default function BrainWaves({
  ...
  connected: boolean;
  source?: "real" | "simulated" | "fake" | "replay";
  label?: string;
  compact?: boolean;
}) {
  ...
  return (
    <div className={"waves" + (compact ? " compact" : "")}>
      <div className="waves-head">
        <span className="waves-title">{label ?? (source === "real" ? "Your brain waves" : source === "replay" ? "Recorded brain waves" : "Practice brain waves")}</span>
        <span className="waves-sub">{!connected ? "no headset" : source === "real" ? "live, microvolts" : source === "replay" ? "replay, microvolts" : source === "simulated" || source === "fake" ? "simulated signal" : "source unverified"}</span>
      </div>
