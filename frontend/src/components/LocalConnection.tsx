...
export default function LocalConnection({ children }: { children: ReactNode }) {
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (response.status === 401) throw new Error("The pairing code has changed. Copy the code from the running Neurospace service.");
      if (!response.ok) throw new Error("Connection refused. Check the allowed website address in your local service settings.");
      const result = await response.json();
      if (result.ok !== true) throw new Error("This is not a Neurospace service. Check the backend address.");
      setConnected(true);
    } catch (e) {
      setError(e instanceof TypeError || (e instanceof DOMException && e.name === "TimeoutError")
        ? "Could not reach Neurospace on this computer. Start the service, allow local network access in your browser, then try again."
  ...
  return <section className="panel" style={{maxWidth: 620, margin: "3rem auto"}}>
    ...
    <p>Neurospace's headset, button, and session data stay with the service on your laptop. Start it before opening your workspace.</p>
    <p>Run this in the Neurospace project folder:</p>
    <pre style={{overflowX: "auto"}}>uv run neurospace serve</pre>
    ...
    <form onSubmit={e => { e.preventDefault(); void connect(); }}>
      ...
      <button type="submit" disabled={busy || !token.trim()}>{busy ? "Connecting…" : "Connect to Neurospace"}</button>
    </form>
