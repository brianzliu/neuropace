...
export default function ModelSettings() {
  ...
  const [configured, setConfigured] = useState<Record<Provider, boolean>>({ openai: false, openrouter: false });
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`${provider === "openai" ? "OpenAI" : "OpenRouter"} is active for new sessions.`);
  ...
  return <div className="transcription-settings model-settings">
    ...
    {loading ? <p>Checking setup…</p> : <form onSubmit={(e) => void save(e)}>
      <div className="provider-switch" role="group" aria-label="Model provider">
        {(["openai", "openrouter"] as Provider[]).map((item) => <button
          key={item} type="button" className={provider === item ? "is-active" : ""} onClick={() => choose(item)}
        >{item === "openai" ? "OpenAI" : "OpenRouter"}{configured[item] ? " · saved" : ""}</button>)}
      </div>
      ...
      <label htmlFor="model-api-key">{provider === "openai" ? "OpenAI" : "OpenRouter"} API key</label>
      ...
      <button className="btn btn-primary" disabled={busy || !models[provider].trim() || (!configured[provider] && !key.trim())}>{busy ? "Saving…" : `Use ${provider === "openai" ? "OpenAI" : "OpenRouter"}`}</button>
    </form>}
  </div>;
}
