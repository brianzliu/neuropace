...
export default function Setup() {
  ...
  const scanRelay = async () => {
    ...
    try {
      ...
      setScanMessage(result.error || (!result.devices.length ? "No Neurospace UNO Q relay found. Start the relay service on your board, then scan again." : "Select your board. Connection is verified in the live session."));
  ...
  const pills: ConnectionPill[] = [
    relayAddress
      ...
      : headsetChoice === "sim" || headsetChoice === "fake"
        ? { key: "eeg", label: "EEG", state: "simulated", detail: headsetChoice === "fake" ? "fake: mindwave pipeline synthetic EEG (labelled)" : "simulated: Neurospace synthetic EEG (labelled)" }
              ...
              <div className="form-grid">
                <label>
                  ...
                  <select value={headsetChoice} onChange={(e) => onHeadsetChoice(e.target.value)}>
                    ...
                    <option value="sim">simulated (Neurospace synthetic EEG)</option>
