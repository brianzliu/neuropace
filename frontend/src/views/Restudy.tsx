      ...
      <div className="lesson single">
        ...
        {headsetOn ? (
          <div className="lesson-waves">
            <BrainWaves source={focus.headset?.kind} samples={focus.raw} bands={focus.bands} connected={headsetOn} compact label={focus.headset?.kind === "real" ? "Your focus while you read" : focus.headset?.kind === "replay" ? "Recorded focus" : "Practice focus"} />
          </div>
        ) : null}
      </div>
    </div>
...
function WhatWorked({ tally }: { tally: TallySummary }) {
  ...
  if (!st || st.rescues === 0) return <p className="sub">Your answers will help us find which explanations work for you.</p>;
  return (
    <p className="sub">
