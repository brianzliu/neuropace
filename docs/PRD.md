...
| FR-L4 | A **tap** on the totem pad (or the `T` key / on-screen button, both labelled simulated) marks a span ending at the tap time with an 8 s lead-in, snapped outward to word boundaries |
...
| FR-L9 | On a **tap**, the catch-up card for the flagged span appears in under 1 s: "You missed: <glance in the learner's best form>. Now: <the last few words spoken>". It is one line, dim, static, and fades after ~6 s (configurable). A key shows the same recap in a second form (demo) |
